          ...
          cartData.map((item, index) => {
            ...
            return (
              <div key={index} className='py-4 border-t border-b text-zinc-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4'>
                ...
                <img onClick={() => {
                  if (confirm('Remove the item from the cart?')) {
                    updateQuantity(item._id, item.size, 0)
                  }
                }}
                  className='w-5 mr-4 sm:w-5 cursor-pointer' src={assets.bin_icon} alt="" />
      ...
      <div className='flex justify-end m-20'>
        <div className=' sm:w-[450px]'>
          ...
          <div className='w-full text-end'>
            <button onClick={() => navigate('/place-order')} className='bg-yellow-500 text-black text-md my-8 px-8 py-3'>PLACE ORDER</button>
          </div>
        </div>
      </div>
